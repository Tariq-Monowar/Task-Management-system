...
class ProjectController {
  ...
  createProject = async (req: CustomRequest, res: Response): Promise<void> => {
    try {
      ...
      const { title, description, timeline, currency, amount, priority } = req.body;
  ...
  getAllProjects = async (req: Request, res: Response): Promise<void> => {
    try {
      let getCash = await getCacheProject();
      if (getCash) {
  ...
  updateProject = async (req: CustomRequest, res: Response): Promise<void> => {
    try {
      ...
      await updateCacheProject(updatedProject);
  ...
  deleteProject = async (req: CustomRequest, res: Response): Promise<void> => {
    try {
      ...
      await removeCacheProject(req.params.id);
  ...
  addUserToProject = async (
    ...
    try {
      ...
      if (newUser && newUser.users.length > 0) {
        await addNewUserCacheProject(projectId, newUser.users[0])
